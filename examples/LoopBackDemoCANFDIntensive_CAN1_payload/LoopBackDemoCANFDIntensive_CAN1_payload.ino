...
//--- Limit hardware Tx Buffers to 32 data bytes
  settings.mHardwareTransmitBufferPayload = ACANFD_FeatherM4CAN_Settings::PAYLOAD_32_BYTES ;
//--- Limit hardware receive FIFO 0 to 24 data bytes
  settings.mHardwareRxFIFO0Payload = ACANFD_FeatherM4CAN_Settings::PAYLOAD_24_BYTES ;
...
//--- Receive frame
  ...
  if (gOk && can1.receiveFD0 (receivedFrame)) {
    ...
    if (storedFrame.type != CANFDMessage::CAN_REMOTE) {
      const auto minPayload = (can1.hardwareTxBufferPayload () < can1.hardwareRxFIFO0Payload ())
        ? can1.hardwareTxBufferPayload ()
        : can1.hardwareRxFIFO0Payload ()
      ;
      const uint32_t lg = ACANFD_FeatherM4CAN_Settings::frameDataByteCountForPayload (minPayload) ;
      const uint32_t comparedLength = (lg < receivedFrame.len) ? lg : receivedFrame.len ;
      for (uint32_t i=0 ; (i<comparedLength) && sameFrames ; i++) {
        sameFrames = storedFrame.data [i] == receivedFrame.data [i] ;
      }
      for (uint32_t i=comparedLength ; (i<receivedFrame.len) && sameFrames ; i++) {
        sameFrames = receivedFrame.data [i] == 0xCC ;
      }
    }
    if (!sameFrames) {
      gOk = false ;
      ...
      Serial.print (" ") ;
      for (uint32_t i=0 ; i<storedFrame.len ; i++) {
        Serial.print (" ") ;
        Serial.print (storedFrame.data [i] >> 4, HEX) ;
        Serial.print (storedFrame.data [i] & 15, HEX) ;
      }
      Serial.println () ;
      Serial.print (" ") ;
      for (uint32_t i=0 ; i<receivedFrame.len ; i++) {
        Serial.print (" ") ;
        Serial.print (receivedFrame.data [i] >> 4, HEX) ;
        Serial.print (receivedFrame.data [i] & 15, HEX) ;
      }
      Serial.println () ;
    }
  }
